...
public class ResetPasswordPresenter extends GodBasePresenter<ResetPasswordView> {
    ...
    public void resetPassword(String mobile, String sms, String loginOldPassword, String loginPassword) {
        String token = accountDAO.getToken();
        Disposable disposable = requestStore.resetPassword(mobile, sms, Md5Utils.md5(loginOldPassword), Md5Utils.md5(loginPassword), token, new Consumer<CommonRespond>() {
            @Override
            public void accept(CommonRespond data) {
                if (data.getCode() == 200) {
