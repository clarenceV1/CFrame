...
import com.meetone.work.base.StateCode;
import com.meetone.work.utils.ShareUtil;
...
public class ShareWechatMomentsManager {
    public static void shareWechatMonentsForProtocol(final Context context, final WebProtocolDO protocolDO, final IWebProtocolCallback callback) {
        if (!PackageUtils.isAppInstall(context, ShareUtil.WX)) {
            callback.callBack(context,protocolDO, StateCode.STATE_CODE_NO_INSTALL, "客户端没有安装微信", "分享出错");
            return;
        }
        shareWechatMomnets(protocolDO.getParams(), new PlatformActionListener() {
            @Override
    ...
    public static void shareWechatMomnets(String parama, PlatformActionListener shareCallback) {
